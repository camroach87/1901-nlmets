---
title: "Subject specific curves for times series forecasting of smart meter demand"
...
all_data_file <- "cache/data_all_2018_sscts.RData"
...
building_attributes = c("basebldngfeedonly",
                        "dxsystem",
                        "electricelementheating",
                        "centraldist")
# TODO: This needs to be updated in more than one place (see test-models.R)
#       whenever predictors change. Not good! Define in one place only.
#       Formula for test_models should be created here based on these vars.
# TODO: Rename to pred_vars because lagged demand is also included.
weather_vars <- c("scaled_temperature",
                  "scaled_temperature_lag_12",
                  "scaled_temperature_lag_24",
                  "scaled_temperature_lag_48",
                  "scaled_temperature_lag_72",
...
if (!file.exists(all_data_file)) {
  ...
  cat("Loaded cached processed data...\n\n")
  load(all_data_file)
}
...
main_df <- main_df %>%
  ...
  mutate(temperature = scaled_temperature,  # keep original temperature for plotting
         wh_lag_96 = scaled_wh_lag_96) %>%  # keep original 96 lag demand for plotting
  mutate_at(vars(starts_with("scaled")), scale_dbl) %>%  # scale
  select(ts, date, period, bid, wh, temperature, wh_lag_96,
...
main_df <- main_df %>% 
  filter(date %in% dates_filter,
         period %in% fcst_test_periods)


# Filter problem data
main_df <- main_df %>% 
  filter(!(bid == "BID0059" & year(date) == 2017 & month(date) %in% 6:7))  # flat readings
...
if (!file.exists(fs_df_file)) {
  cat("Running feature selection for max of", n_max_features,
      "candidate predictors...\n\n")
  start_time <- Sys.time()
  fs_df <- list(month = unique(month(fcst_test_dates)),
                period = fcst_test_periods) %>% 
    cross_df() %>% 
    mutate(fs_date = map(month,
                         ~ min(fcst_test_dates[month(fcst_test_dates) == .x]))) %>% 
    unnest() %>% 
    mutate(features = future_map2(fs_date, 
                                  period,
                                  select_features,
  ...
  cat("Loaded cached feature selection file...\n\n")
  load(fs_df_file)
}
...
if (!file.exists(model_df_file)) {
  ...
  model_df <- list(period = fcst_test_periods,
                   fcst_date = fcst_test_dates) %>% 
    cross_df() %>% 
    mutate(fcst_date = as_date(fcst_date)) %>% 
    mutate(fit_results = future_map2(fcst_date, period, test_models, 
...
This paper is structured as follows. Section \@ref(data) describes the data we are working with. Section \@ref(methodology) gives a detailed description of the models we are working with and how they are assessed. Results of our forecasts and a scenario analysis example are presented in Section \@ref(results). Concluding remarks are given in Section \@ref(conclusion).


# Data

We have time series and attribute data for `r n_buildings` commercial office buildings located across Australia. We focus on business days only in our analysis as these are significantly more important than non-business days for energy management. Non-business days typically have far less demand than business days as equipment is non-operational. Note that our approach can be applied to non-business days as well.
...
```{r temp-wh-scatter, fig.cap="Relationship between temperature and electricity demand for four Australian office buildings during Summer. Data for at four different 15-minute periods of the day are shown. We can see a clear change in the relationship of our predictor and response variables both between buildings and at different times of the day."}

plot_bids <- c(sample(unique(main_df$bid), 3), "BID0052A")
plot_periods <- c(24, 48, 72, 96)

main_df %>%
  filter(bid %in% plot_bids,
  ...
  labs(y = expression(Log~normalised~electricity~(log(Wh/m^2))),
       x = "Temperature (°C)") +
  theme(legend.position = "none")
```


```{r wh-lagwh96-scatter, fig.cap="Relationship between electricity demand and one-day lagged demand for all Australian office buildings during Summer. Mondays have a different relationship with their one-day lagged demand values due to the discontinuity in time series introduced by removing non-business days. This is most noticeable during business hours (periods 48 and 72)."}
main_df %>%
  filter(month(ts) %in% c(12, 1, 2),
         period %in% plot_periods) %>% 
  mutate(wday = wday(ts, label = TRUE, abbr = FALSE)) %>% 
  ggplot(aes(x = wh_lag_96, y = wh, colour = wday)) + 
  geom_point(shape = "O", alpha = 0.25) + 
  geom_smooth(alpha = 0.2, method = "lm", formula = y ~ ns(x, df = 3)) +
  facet_wrap(~period) +
  labs(y = expression(Normalised~electricity~(Wh/m^2)),
       x = expression(One~day~lagged~normalised~electricity~(Wh/m^2))) +
  ylim(0, NA) + 
  xlim(0, NA)
```
...
p_data <- p_data %>%
  mutate(`Linear spline` = predict(lm_ls, .),
...
ggplot(mapping = aes(x = temperature)) +
  ...
  geom_line(data = p_data %>% 
              select(temperature, `Linear spline`, `Cubic spline`,
                     `Natural spline`) %>% 
...
To justify our final model that we use for scenario analysis we test several models that can be thought of as simpler versions. Each model has a change introduced and the improvement in performance is used as justification for each. As a starting benchmark, we fit individual linear regression models to each building. The second set of individual models incorporate natural splines to model the temperature and demand relationships. The third, fourth and fifth models are random intercept, random slope and subject specific curve models. Finally, the sixth model is a subject specific curve model that includes building attributes as fixed effects. We also fit a naive model that uses the previous day's observed values as a benchmark. This is summarised in Table \@ref(tab:model-frameworks).
...
Individual natural splines   | INS  | Natural spline models fit to each building | Feature selection
Pooled regression            | PR | Regression model. Used for feature selection | All/Feature selection
Random intercept             | RI   | Mixed effects model with random intercept | Feature selection
Random intercept and slope   | RI   | Mixed effects model with random intercept and slope | Feature selection
Subject specific curves      | SSC  | Mixed effects model with subject specific curves | Feature selection
Subject specific curves with attributes | SSCATTR | SSC with building attributes included as fixed effects | Feature selection
...
These serve as benchmark models to determine if moving to a mixed models framework improves prediction accuracy. Separate models are fit for each building. Note that a subscript for building has been omitted from each of these individual models to improve clarity.

#### Naive forecast model

The simplest benchmark is a naive forecasting model, where the previous day's values are used. This is often a surprisingly effective forecasting approach [@Hyndman2018-ok]. The demand for a building at time $t$ is given by

$$
y_{t} = y_{t-24 \text{ hours}} + \epsilon_{t}, \quad \epsilon_{t} \sim N(0,\sigma^2).
$$

Note that since we have restricted ourselves to business days only, $t-24 \text{ hours}$ is a slight abuse of notation and is used to represent the observed values from the last _business_ day. So a forecast for Monday will use observed values from the previous Friday. Using observed values from Sunday would produce a much weaker benchmark due to different demand dynamics on working and non-working days.

#### Individual linear regression model

A simple benchmark model is created by fitting a linear regression model to each building and period of the day. The demand for a building at time $t$ is given by

$$
\log y_{t} = \beta_{0,p} + \beta_{1,p} w_{0,t} + \epsilon_{t}, \quad \epsilon_{t} \sim N(0,\sigma^2_{p}),
$$

where $p$ is the 15-minute period of the day at time $t$, $w_{0,t}$ is the scaled temperature experienced^[The 0 subscript denotes no lag and is included to keep notation consistent with Table \@ref(tab:weather-vars).] at time $t$ and $\epsilon_{t}$ is the residual. We call these our "Individual Linear Regression" (ILR) models.
...
* Much of the literature on load forecasting already identifies the importance of current temperature in forecasting demand and we can see a clear nonlinear relationship in Figure \@ref(fig:temp-wh-scatter).
...
We use the pooled linear regression model for feature selection as it is quick to fit using ordinary least squares and allows us to model buildings by using a dummy variable for each. Using a linear model also has the advantage of allowing us to efficiently compute the leave-one-out cross-validation (LOOCV) scores [@Seber2012-gu]. Training data is comprised of business days within a window of `r fcst_train_window` days prior to the month we wish to select variables for. As our experimental setup for the validation phase involves one-day ahead forecasts, this variable selection prevents us from using any data from the future. For example, when forecasting for any date in January, only data from the months of October, November and December would have been used to select predictors.

We use best subset selection during feature selection. So, given $p$ predictors we choose the combination of these that produce the best $R^2$ scores. Once the best model based on $R^2$ has been determined for each set of $p$ predictors, we use the LOOCV score to determine the best. The LOOCV score is chosen as it gives an estimate of the out of sample performance of our models. Figure \@ref(fig:fs-model-selection) shows the LOOCV scores for each predictor set of size $p$. During feature selection we chose to avoid greedy approaches such as forward or backward stepwise selection; or approaches that work systematically through lagged weather variables [@Hyndman2010-ui]. Naturally, greedy methods have computational benefits, but it is interesting to observe which features are chosen when _all_ possible predictor combinations are assessed.
...
fs_df %>% 
  ...
  geom_line(alpha = 0.5) +
  facet_wrap(~month) + 
  scale_colour_gradient(low = ba_palette[1], high = ba_palette[2]) + 
  ylim(0,0.25) +
...
$w_{b,72,t}$    | 72      | Scaled temperature lagged by 18 hours.
...
These are all well established forecasting metrics. Advantages and disadvantages of each are well described in @Hyndman2006-bp.

When comparing these metrics in Section \@ref(forecasting-accuracy), we do find that the SSCATTR model produces the best point forecasts. To establish that this result is statistically significant we also carry out Diebold-Mariano tests against the ILR model in Section \@ref(diebold-mariano-test).
...
(ref:fs-alpha) Feature selection for summer and winter including shoulder months.

```{r fs-alpha, fig.cap="(ref:fs-alpha)", fig.height=7*1.4}
fs_df %>% 
  ...
  right_join(list(month = unique(month(fcst_test_dates)),
                  period = fcst_test_periods) %>% 
               cross_df() %>% 
               mutate(month = month(month, label = TRUE)),
             by = c("month", "period")) %>% 
  ...
  mutate(selected = tidyr::replace_na(selected, FALSE),
         vars = fct_rev(factor(vars, levels = weather_vars)),
  ...
  mutate(selected = if_else(n_selected > 0, TRUE, FALSE),
         predictor_type = case_when(
           str_detect(vars, "^scaled_temperature") ~ "Temperature",
           str_detect(vars, "^scaled_wh") ~ "Demand",
           TRUE ~ "Other"
         )) %>% 
  filter(n_selected > 0) %>%  # don't want any fill for 0
  ggplot(aes(x = period, y = vars, alpha = n_selected, 
             fill = predictor_type)) +
  geom_tile() +
...
Table \@ref(tab:accuracy-results) shows the MAE, MAPE, sMAPE and MASE for each model across the entire day, during business hours (7:00 am to 7:00 pm) and during non-business hours (7:00 pm to 7:00 am). The INS models have the worst forecasting accuracy across all metrics. This is likely due to overfitting - the natural splines are too flexible for some of the buildings with the amount of data used for training. Evidence of this can be seen in Figure \@ref(fig:one-day-ahead-forecast) where erratic spikes in the predicted demand can be seen. When moving to a mixed effects model that uses natural splines the predictive accuracy is greatly improved. The SSC and SSCATTR models consistently outperform all other models. Importantly, accuracy further increases when building attributes are included as fixed effects in the SSCATTR model.

Given that the SSCATTR model outperforms each of our benchmarks it seems reasonable to conclude that forecasting with mixed effects models is a reasonable practice that should be encouraged when data is available from similar subjects.
...
calculate_accuracy <- function(df, periods) {
  ...
  naive_mae <- mase_df %>% filter(model == "Naive") %>% pull(mae)
  
  mase_df <- mase_df %>% 
    mutate(mase = mae/naive_mae)
  
  accuracy_df <- df %>% 
    summarise(mape = 100*mean(abs(resid/wh)),
...
tribble(
  ...
  unnest(accuracy) %>% 
  select(Model = model, MAE = mae, MAPE = mape, sMAPE = smape, 
         MASE = mase) %>% 
...
(ref:one-day-ahead-forecast-cap) One-day ahead forecasts for `r format(p_one_day_ahead_date, "%d %B, %Y")`.

```{r one-day-ahead-forecast, fig.cap="(ref:one-day-ahead-forecast-cap)", fig.height=7*1.4}
model_df %>% 
  unnest(preds) %>%
...
model_df %>%
  ...
  rename(Period = period, `DM statistic` = dm_statistic,
...
<!-- (ref:dm-test-all-cap) Diebold-Mariano test to compare forecast accuracy of ILR and SSCATTR models. Alternative hypothesis is SSCATTR model is more accurate than ILR model. All periods tested together. -->
...
<!--          dm_statistic = map_dbl(dm_test, "statistic"), -->
<!--          dm_p_value = map_dbl(dm_test, "p.value")) %>% -->
<!--   select(dm_statistic, dm_p_value) %>% -->
<!--   mutate(significance = case_when( -->
<!--     dm_p_value < 0.001 ~ "***", -->
<!--     dm_p_value < 0.01 ~ "**", -->
<!--     dm_p_value < 0.05 ~ "*", -->
...
Having confirmed that our mixed models produce satisfactory predictions compared to individual provides us with justification for using them for scenario analysis. We can investigate how changing certain variable values increases or decreases energy demand in buildings. We fit the SSCATTR model on all training data before producing predictions. This is an ex-post forecasting scenario since we are using known data for our predictor variables when predicting demand.
...
(ref:attribute-changes) Expected normalised electricity consumption for three days in January, 2017 for building `r scenario_bid`. The expected demand for the existing building is shown ("Original building"), as well as four scenarios each of which involves the modification of one of the attributes. The grey line show the actual demand observed on each day.
...
p_data <- model_df %>% 
  ...
  # TODO: Move this to the test-models script
  # mutate(scenario = factor(scenario, labels = c("Original building", "Tenant feed included", "Switch to DX system", "Switch to electric element heating", "Switch to centralised distribution"))) %>% 

ggplot() +
  geom_line(data = p_data, aes(x = ts, y = wh),
            colour = "grey") +
...
scenario_vars <- c("Original building", "Switch to DX system")
...
p_df <- model_df %>% 
  ...
  # TODO: Move this to the test-models script
  mutate(scenario = factor(scenario, levels = c("original", "dxsystem"),
                           labels = scenario_vars)) %>% 
  select(scenario, ts, pred) %>% 
  spread(scenario, pred) %>% 
  mutate(saving = get(scenario_vars[2]) - get(scenario_vars[1]), 
  # TODO: is this saving calc correct? Should I divide by 4?
         saving = cumsum(saving)) %>% 
  gather(var, Wh, -ts) %>% 
  mutate(facet_row = if_else(var %in% scenario_vars, "Energy usage",
                             "Cumulative difference"),
         facet_row = factor(facet_row, levels = c("Energy usage",
                             "Cumulative difference")))
...
ggplot(mapping = aes(x = ts, y = Wh)) +
  geom_line(data = p_df %>% 
              ...
              rename(scenario = var),
            aes(colour = scenario)) +
  geom_area(data = filter(p_df, var == "saving"), fill = ba_palette[3],
            alpha = 0.5) +
...
```{r fs-all-periods-months, fig.cap="(ref:fs-all-periods-months)", fig.height = 7*1.4}
fs_df %>% 
  select(period, month, vars) %>% 
  unnest() %>% 
  mutate(selected = TRUE,
