...
```{r one-day-ahead-forecast, fig.cap="(ref:one-day-ahead-forecast-cap)", fig.height=8*1.4}
model_df %>%
...
```{r fs-all-periods-months, fig.cap="(ref:fs-all-periods-months)", fig.height = 8*1.4}
fs_df %>%
  select(period, month, vars) %>%
  unnest() %>%
  mutate(
