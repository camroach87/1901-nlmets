...
ggplot(mapping = aes(x = temperature)) +
  geom_point(data = p_data, aes(y = log(wh)), shape = "O") +
  geom_line(
    data = p_data %>%
      select(
...
$$
where $f_p$ is a smooth function modelling the relationship between $w_{0,t}$ and the logged demand for period $p$. The function $f_p$ is modelled using natural cubic splines with three degrees of freedom. Knots are set at the 33.3^rd^ and 66.6^th^ quantiles of $w_{0,t}$. We refer to this set of models as the "Individual Natural Spline" (INS) models.
...
\epsilon_{b,t} \sim N(0, \sigma^2_{\epsilon, p}).
\end{gathered}
$$
...
$$
\begin{gathered}
g_{b,p}(x) = u_{b,p,0} + \sum^3_{k=1} u_{b,p,k} B_{p,k}(x), \\
\left( u_{b,p,0}, \ldots, u_{b,p,3} \right) \sim N(0, \Sigma),
\end{gathered}
$$
...
fs_df %>%
  ...
  ggplot(aes(x = period, y = description, fill = n_selected)) +
  geom_tile() +
  theme(
  ...
  scale_fill_gradient2(low = "white", high = ba_palette[1])
```
